...
const approveJobApplication = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
...
const viewAllApplication = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    ...
    res
      .status(200)
      .json(new ApiResponse(200, { allApplications }, "Applications Fetched"));
  }
);
...
const getMyJobs = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    ...
    res
      .status(200)
      .json(
        new ApiResponse(200, { myJobsPost, totalDocument }, "All Jobs Fetched")
      );
  }
);
...
const viewMyApplications = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    ...
    const myApplications = await Application.find({ appliedBy: userCheck._id })
      .sort({ [sortOption]: 1 })
      .skip(skip)
      .limit(perPage)
