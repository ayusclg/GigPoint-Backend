...
class RatingController {
  ...
  myRating = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const page = parseInt(req.query.page as string) || 1;
      const perPage = parseInt(req.query.perPage as string) || 5;

      const rating = await Rating.find({ raterUserId: req.userId })
      ...
      const total = await Rating.countDocuments({ raterUserId: req.userId });
  ...
  deleteRating = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      if (!isAuthorized) throw new ApiError(403, "Permission Denied");
      ...
      res
        .status(200)
        .json(new ApiResponse(200, rating.comment, "Rating Deleted"));
    }
  );

  showRecentRating = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const user = await User.findById(req.userId);
      if (!user || !isWorker(user))
        throw new ApiError(403, "Permission Denied");

      const displayRatings = await Rating.find({ ratedUserId: user._id })
        .populate("raterUserId", "fullName profilePicture")
        .sort({ createdAt: "asc" })
        .limit(4)
        .select("-ratedUserId")
      ...
      res
        .status(200)
        .json(
          new ApiResponse(200, displayRatings, "Ratings Fetched Successfully")
        );
    }
  );

  viewAllRating = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const page = parseInt(req.query.page as string) || 1;
      const perPage = parseInt(req.query.perPage as string) || 5;
