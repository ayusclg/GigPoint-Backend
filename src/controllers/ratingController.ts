...
const createRating = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    if(existingRating) throw new ApiError(403,"You Have Already Rated This Worker")
...
const myRating = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    }).populate("raterUserId", "fullName").populate("ratedUserId", "fullName phoneNo").limit(perPage).skip((page - 1) * perPage).lean()
    ...
    res.status(200).json(new ApiResponse(200,response,"Rating Fetched Successfully"))
})

export {createRating,myRating}